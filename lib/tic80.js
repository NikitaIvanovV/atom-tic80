...
class Tic80 {
  ...
  */
  ...
  static isSupportedVersion(version) {
    ...
    return version.major >= this.minimalTicVersion.major || version.minor >= this.minimalTicVersion.minor || version.patch >= this.minimalTicVersion.patch;
  }

  async run(skip=true, fullscreen=false, {cartPath, fileSystemPath, commands, cli, showGreetingMessage, checkVersion, noSound, scale, additionalArgs}) {
