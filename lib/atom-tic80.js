...
export default {
  ...
  async runTic80(params = {}, hideTerminal=false, events = {}) {
    ...
    try {
      await this.tic80.run(
        getConfig('tic80.skipStartUpAnimation'),
        getConfig('tic80.fullscreen'),
        {...params, showGreetingMessage: this.showGreetingMessage},
        events
      );
    } catch (e) {
...
${Tic80.languages.map(lang => `\`${lang.extension}\``).join(', ')}
      ...
      if (! this.pathsWithAddedMetadata.includes(filePath)) {
        ...
        const comment = ticLanguages.fromExtension(extName).comment;
