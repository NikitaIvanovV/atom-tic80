...
// Add autocompletion only for these because they have they language packages for Atom
const languages = [
  ticLanguages.Lua,
...
function makeFunctionSnippet(name, argNames, leftBlock, rightBlock, separator, {prefix, emptyArgsSuffix} = {}) {
  const args = [];
...
function setFunctionSnippet(apiEntry, name, language) {
  ...
  switch (language) {
    ...
    case ticLanguages.Moon:
      snippet = makeFunctionSnippet(name, args, ' ', '', ', ', {emptyArgsSuffix: '!'});
      break;
    case ticLanguages.Wren:
      snippet = makeFunctionSnippet(name, args, '(', ')', ', ', {prefix: 'TIC.'});
      break;
    default:
      throw new Error("Uknown language: " + language.name);
  }
  
  apiEntry.snippet[language.ticName] = snippet;
}
...
function getCompletions() {
  ...
  const apiDocs = cson.requireCSONFile(path.join(__dirname, '..', 'api-docs.cson'));
  ...
  for (const [name, apiEntry] of Object.entries(apiDocs)) {
    ...
    if (apiEntry.type === 'function' && apiEntry.snippet === undefined) {
      apiEntry.snippet = {};
      if (apiEntry.args === undefined) {
        apiEntry.args = [];
      }
      for (const lang of languages) {
        setFunctionSnippet(apiEntry, name, lang);
      }
    }
    
    // Delete properties used for generating other properties
    delete apiEntry.args;
  
    // Set other types to function to add a neat icon :)
    if (apiEntry.type === 'other') {
      apiEntry.type = 'function';
    }
